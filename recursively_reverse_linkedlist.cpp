...
node *recursive_reverse(node *&head)
{
	...
	// rec case
	node *small_head = recursive_reverse(head -> next) ; // smallhead is 4

	node *temp = small_head ; // temp will be 4
	while(temp -> next != NULL)
	{
		temp = temp -> next ;
	}
	head -> next = NULL ; // 3 will point to NULL
	temp -> next = head ; // and 4 will point to 3 
	return small_head ; // returns 4
}

node *optimised_reverse(node *&head)
{
	// smallest linked list
	// corner case
	if (head -> next == NULL or head == NULL)
	...
	// in this prgm, while loop is removed which has improved the overall complexity
	node *temp = head -> next ;
	temp -> next = head ;
	head -> next = NULL ;
	return small_head ;
